...
class DTLSD(nn.Module):
    ...
    def forward(self, samples: NestedTensor, targets:List=None):
        ...
        out = {'pred_logits': outputs_class[-1], 'pred_lines': outputs_coord_list[-1]}
        if self.aux_loss:
        ...
        if hs_enc is not None:
            ...
            out['interm_outputs'] = {'pred_logits': interm_class, 'pred_lines': interm_coord}
            out['interm_outputs_for_matching_pre'] = {'pred_logits': interm_class, 'pred_lines': init_box_proposal}
            ...
            if hs_enc.shape[0] > 1:
                ...
                out['enc_outputs'] = [
                    {'pred_logits': a, 'pred_lines': b} for a, b in zip(enc_outputs_class, enc_outputs_coord)
                ]
...
class SetCriterion(nn.Module):
    ...
    def forward(self, outputs, targets, return_indices=False):
        ...
        if self.training and dn_meta and 'output_known_lbs_lines' in dn_meta:
            output_known_lbs_lines,single_pad, scalar = self.prep_for_dn(dn_meta)
            ...
            for loss in self.losses:
                ...
                if 'labels' in loss:
                    kwargs = {'log': False}
                l_dict.update(self.get_loss(loss, output_known_lbs_lines, targets, dn_pos_idx, num_boxes*scalar,**kwargs))

            l_dict = {k + f'_dn': v for k, v in l_dict.items()}
            losses.update(l_dict)
        else:
            l_dict = dict()
            l_dict['loss_line_dn'] = torch.as_tensor(0.).to('cuda')
        ...
        if 'aux_outputs' in outputs:
            for idx, aux_outputs in enumerate(outputs['aux_outputs']):
                ...
                if self.training and dn_meta and 'output_known_lbs_lines' in dn_meta:
                    aux_outputs_known = output_known_lbs_lines['aux_outputs'][idx]
                    l_dict={}
                    for loss in self.losses:
                        kwargs = {}
                        if 'labels' in loss:
    ...
    def prep_for_dn(self,dn_meta):
        ...
        return output_known_lbs_lines,single_pad,num_dn_groups
...
@MODULE_BUILD_FUNCS.registe_with_name(module_name='dtlsd')
def build_dtlsd(args):
    ...
    # prepare weight dict
    weight_dict = {'loss_ce': args.cls_loss_coef, 'loss_line': args.lines_loss_coef}
    clean_weight_dict_wo_dn = copy.deepcopy(weight_dict)

    
    # for DN training
    if args.use_dn:
        weight_dict['loss_line_dn'] = args.lines_loss_coef
    ...
    criterion.to(device)
    postprocessors = {'line': PostProcess(num_select=args.num_select, nms_iou_threshold=args.nms_iou_threshold)}
