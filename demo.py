...
def main(args):
    model_config_path = "config/DTLSD/DTLSD_4scale_swin.py" # change the path of the model config file
    model_checkpoint_path = "DTLSD_checkpoint0035.pth" # change the path of the model checkpoint
    ...
    # load image
    files = ['00031723', '00031797', 'P1080018', 'P1080074']
    for file in files:
        try:
            raw_img = Image.open(f'./samples/{file}.png').convert("RGB")
        except:
            raw_img = Image.open(f'./samples/{file}.jpg').convert("RGB")
        w, h = raw_img.size
        ...
        plt.axis('off')
        plt.axis('equal')
        plt.savefig(f'./results/{file}_VIS.png', pad_inches=0.1, bbox_inches='tight')
        # plt.show()


if __name__ == '__main__':
    parser = argparse.ArgumentParser('DTLSD', parents=[get_args_parser()])
