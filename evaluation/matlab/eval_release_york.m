function eval_release_york(image_path, line_gt_path, output_file, result_path, output_size, mode)
...
for index=1:numResults
  ...
  gtlines = load(gtname);
  gtlines = gtlines.lines ./ [641, 481] .* [640, 480];
  gtlines = reshape(gtlines', 4, [])';
  ne = size(gtlines,1);
  ...
  parfor m=1:nLineThresh
    ...
    %disp([size(edgemap0), size(edgemap1)])
