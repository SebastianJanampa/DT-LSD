...
for ((i=0;i<${#epoch[@]};++i)); do
	...
	python main.py \
	  --output_dir $output \
		-c config/DTLSD/DTLSD_5scale.py --coco_path data/wireframe_processed  \
	...
	python main.py \
	  --output_dir $output \
		-c config/DTLSD/DTLSD_5scale.py --coco_path data/york_processed  \
		--eval --resume $folder/checkpoint0${epoch[i]}.pth --benchmark --dataset val --append_word ${epoch[i]} \
		--options dn_scalar=100 embed_init_tgt=TRUE \
		dn_label_coef=1.0 dn_bbox_coef=1.0 use_ema=False \
		dn_box_noise_scale=1.0 num_classes=2 dn_labelbook_size=2
